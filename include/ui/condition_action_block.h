...
class ConditionActionBlock : public ActionBlock {
    Q_OBJECT
    Q_PROPERTY(QString conditionType READ conditionType WRITE setConditionType NOTIFY conditionTypeChanged)
    ...
    Q_PROPERTY(double positionTolerance READ positionTolerance WRITE setPositionTolerance NOTIFY positionToleranceChanged)
    Q_PROPERTY(double angleTolerance READ angleTolerance WRITE setAngleTolerance NOTIFY angleToleranceChanged)
    Q_PROPERTY(QString sensorName READ sensorName WRITE setSensorName NOTIFY sensorNameChanged)
    Q_PROPERTY(double sensorThreshold READ sensorThreshold WRITE setSensorThreshold NOTIFY sensorThresholdChanged)
    Q_PROPERTY(ActionBlock* trueAction READ trueAction WRITE setTrueAction NOTIFY trueActionChanged)
    Q_PROPERTY(ActionBlock* falseAction READ falseAction WRITE setFalseAction NOTIFY falseActionChanged)
...
public:
    ...
    QString name() const override { return tr("条件动作"); }
    QString description() const override { return tr("根据条件执行不同的动作"); }
    QIcon icon() const override;
    bool isRunning() const override { return d_->is_executing; }
    ...
    // 属性访问
    QString conditionType() const { return d_->condition_type; }
    double targetX() const { return d_->target_x; }
    double targetY() const { return d_->target_y; }
    double targetTheta() const { return d_->target_theta; }
    double positionTolerance() const { return d_->position_tolerance; }
    double angleTolerance() const { return d_->angle_tolerance; }
    QString sensorName() const { return d_->sensor_name; }
    double sensorThreshold() const { return d_->sensor_threshold; }
    ActionBlock* trueAction() const { return d_->true_action; }
    ActionBlock* falseAction() const { return d_->false_action; }
