#include "ui/teleoperation_panel.h"
...
void TeleoperationPanel::setRobotController(const std::shared_ptr<RobotController>& controller)
{
    ...
    if (d_->controller) {
        connect(d_->controller.get(), &RobotController::velocityChanged,
                this, &TeleoperationPanel::updateVelocityDisplay);
    }
}
...
void TeleoperationPanel::createControlGroup()
{
    ...
    d_->emergency_stop_button->setStyleSheet("QPushButton { background-color: red; color: white; }");
...
{
    connect(d_->control_mode_combo, QOverload<int>::of(&QComboBox::currentIndexChanged),
            this, &TeleoperationPanel::onControlModeChanged);
    
    connect(d_->speed_limit_spin, QOverload<double>::of(&QDoubleSpinBox::valueChanged),
            this, &TeleoperationPanel::onSpeedLimitChanged);
    
    connect(d_->accel_limit_spin, QOverload<double>::of(&QDoubleSpinBox::valueChanged),
            this, &TeleoperationPanel::onAccelLimitChanged);
    
    connect(d_->emergency_stop_button, &QPushButton::clicked,
            this, &TeleoperationPanel::onEmergencyStop);
    
    connect(d_->joystick, &JoystickWidget::linearJoystickMoved,
            this, &TeleoperationPanel::onJoystickMoved);
}
...
{
    if (!d_->controller || !d_->joystick_group->isEnabled())
        return;
...
{
    d_->velocity_label->setText(tr("当前速度: %1 m/s, %2 rad/s")
        .arg(linear, 0, 'f', 2)
        .arg(angular, 0, 'f', 2));
...
void TeleoperationPanel::updateKeyboardControl()
{
    ...
    if (d_->key_pressed.contains(Qt::Key_Up))
        linear += d_->keyboard_linear_speed;
    if (d_->key_pressed.contains(Qt::Key_Down))
        linear -= d_->keyboard_linear_speed;
    if (d_->key_pressed.contains(Qt::Key_Left))
        angular += d_->keyboard_angular_speed;
    if (d_->key_pressed.contains(Qt::Key_Right))
        angular -= d_->keyboard_angular_speed;
