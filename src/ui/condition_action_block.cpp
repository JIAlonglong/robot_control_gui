...
ConditionActionBlock::ConditionActionBlock(const std::shared_ptr<RobotController>& controller, QObject* parent)
    : ActionBlock(parent)
    , d_(std::make_unique<Private>())
{
    ...
    // 连接必要的信号和槽
    connect(d_->robot_controller.get(), &RobotController::obstacleDetected,
            this, &ConditionActionBlock::onObstacleDetected);
    connect(d_->robot_controller.get(), &RobotController::batteryLevelChanged,
            this, &ConditionActionBlock::onBatteryLevelChanged);
...
void ConditionActionBlock::setTrueAction(ActionBlock* action)
{
    ...
    if (d_->true_action) {
        disconnect(d_->true_action, &ActionBlock::started,
                  this, &ConditionActionBlock::onActionStarted);
        disconnect(d_->true_action, &ActionBlock::completed,
                  this, &ConditionActionBlock::onActionCompleted);
        disconnect(d_->true_action, &ActionBlock::failed,
                  this, &ConditionActionBlock::onActionError);
    }

    d_->true_action = action;

    if (d_->true_action) {
        connect(d_->true_action, &ActionBlock::started,
                this, &ConditionActionBlock::onActionStarted);
        connect(d_->true_action, &ActionBlock::completed,
                this, &ConditionActionBlock::onActionCompleted);
        connect(d_->true_action, &ActionBlock::failed,
                this, &ConditionActionBlock::onActionError);
    }
...
void ConditionActionBlock::setFalseAction(ActionBlock* action)
{
    ...
    if (d_->false_action) {
        disconnect(d_->false_action, &ActionBlock::started,
                  this, &ConditionActionBlock::onActionStarted);
        disconnect(d_->false_action, &ActionBlock::completed,
                  this, &ConditionActionBlock::onActionCompleted);
        disconnect(d_->false_action, &ActionBlock::failed,
                  this, &ConditionActionBlock::onActionError);
    }
    ...
    if (d_->false_action) {
        connect(d_->false_action, &ActionBlock::started,
                this, &ConditionActionBlock::onActionStarted);
        connect(d_->false_action, &ActionBlock::completed,
                this, &ConditionActionBlock::onActionCompleted);
        connect(d_->false_action, &ActionBlock::failed,
                this, &ConditionActionBlock::onActionError);
    }
...
void ConditionActionBlock::execute()
{
    ...
    try {
        ...
        // 建立新的连接
        connect(action_to_execute, &ActionBlock::completed,
                this, &ConditionActionBlock::onActionCompleted,
                Qt::UniqueConnection);
        connect(action_to_execute, &ActionBlock::failed,
                this, &ConditionActionBlock::onActionError,
                Qt::UniqueConnection);
...
void ConditionActionBlock::connectSignals()
{
    ...
    connect(d_->robot_controller.get(), &RobotController::poseUpdated,
            this, &ConditionActionBlock::onPoseUpdated);
    connect(d_->robot_controller.get(), &RobotController::obstacleDetected,
            this, &ConditionActionBlock::onObstacleDetected);
    connect(d_->robot_controller.get(), &RobotController::batteryLevelChanged,
            this, &ConditionActionBlock::onBatteryLevelChanged);
...
bool ConditionActionBlock::evaluateCondition()
{
    ...
    if (d_->condition_type == "at_position") {
        geometry_msgs::PoseWithCovarianceStamped current_pose = d_->robot_controller->getCurrentPose();
...
#include "ui/condition_action_block.moc" 