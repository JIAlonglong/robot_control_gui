...
{
    if (qFuzzyCompare(d_->linear_speed_, speed))
        return;
...
{
    if (qFuzzyCompare(d_->angular_speed_, speed))
        return;
...
{
    if (qFuzzyCompare(d_->max_linear_speed_, speed))
        return;
...
{
    if (qFuzzyCompare(d_->max_angular_speed_, speed))
        return;
...
void SpeedDashboard::drawSpeedScale(QPainter* painter, const QRectF& rect, double max_speed)
{
    ...
    for (int i = 0; i <= num_major_ticks; ++i) {
        ...
        QPointF outer(center.x() + radius * qCos(rad),
                     center.y() - radius * qSin(rad));
        QPointF inner(center.x() + radius * 0.9 * qCos(rad),
                     center.y() - radius * 0.9 * qSin(rad));
...
void SpeedDashboard::drawSpeedNeedle(QPainter* painter, const QRectF& rect, double speed, double max_speed)
{
    const double start_angle = 225;
    const double span_angle = 270;
    ...
    // 绘制指针
    QPointF needle_tip(center.x() + radius * qCos(rad),
                      center.y() - radius * qSin(rad));
